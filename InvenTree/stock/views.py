...
class StockExportOptions(AjaxView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = {"form_valid": True, "format": fmt, "cascade": cascade}
...
class StockAdjust(AjaxView, FormMixin):
    ...
    def do_move(self, destination, set_loc=None):
        ...
        if count == 0:
            ...
            return _(
                "Moved {n} items to {dest}".format(n=count, dest=destination.pathstring)
            )
...
class StockItemSerialize(AjaxUpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if valid:
            try:
                item.serializeStock(
                    quantity, numbers, user, notes=notes, location=destination
                )
            except ValidationError as e:
                    ...
                    if len(sn) > 0:
                        try:
                            ...
                            if len(existing) > 0:
                                ...
                                data["success"] = _(
                                    "Created {n} new stock items".format(n=len(serials))
                                )
                                valid = True
