...
class SerializeStockForm(forms.ModelForm):
    ...
    destination = TreeNodeChoiceField(
        ...
        label="Destination",
        required=True,
        help_text="Destination for serialized stock (by default, will remain in current location)",
    )
    ...
    note = forms.CharField(
        label="Notes", required=False, help_text="Add transaction note (optional)"
    )
...
class ExportOptionsForm(HelperForm):
    ...
    include_sublocations = forms.BooleanField(
        required=False,
        initial=True,
        help_text=_("Include stock items in sub locations"),
    )
...
class AdjustStockForm(forms.ModelForm):
    ...
    note = forms.CharField(
        label="Notes", required=True, help_text="Add note (required)"
    )
    ...
    confirm = forms.BooleanField(
        required=False,
        initial=False,
        label="Confirm stock adjustment",
        help_text=_("Confirm movement of stock items"),
    )

    set_loc = forms.BooleanField(
        required=False,
        initial=False,
        label="Set Default Location",
        help_text=_("Set the destination as the default location for selected parts"),
    )
