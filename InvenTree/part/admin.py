...
class PartResource(ModelResource):
    ...
    # ForeignKey fields
    category = Field(
        attribute="category", widget=widgets.ForeignKeyWidget(PartCategory)
    )

    default_location = Field(
        attribute="default_location", widget=widgets.ForeignKeyWidget(StockLocation)
    )

    default_supplier = Field(
        attribute="default_supplier", widget=widgets.ForeignKeyWidget(SupplierPart)
    )
    ...
    in_stock = Field(
        attribute="total_stock", readonly=True, widget=widgets.IntegerWidget()
    )

    on_order = Field(
        attribute="on_order", readonly=True, widget=widgets.IntegerWidget()
    )

    used_in = Field(
        attribute="used_in_count", readonly=True, widget=widgets.IntegerWidget()
    )

    allocated = Field(
        attribute="allocation_count", readonly=True, widget=widgets.IntegerWidget()
    )

    building = Field(
        attribute="quantity_being_built", readonly=True, widget=widgets.IntegerWidget()
    )
...
class ParameterResource(ModelResource):
    ...
    template = Field(
        attribute="template", widget=widgets.ForeignKeyWidget(PartParameterTemplate)
    )
